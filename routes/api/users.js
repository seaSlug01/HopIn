    ...
    const friends = await User.find(
      {
        ...
        followers: { $in: [userLoggedInId] },
        following: { $in: [userLoggedInId] },
      },
      fieldsNeeded
    )
...
router.get("/suggestions", async (req, res) => {
  ...
  const fans = await User.find({
    _id: { $ne: userLoggedInId },
    following: { $nin: [userLoggedInId], followers: { $in: [userLoggedInId] } },
  })
